...
# this is for almalinux 10, oracle 10 or rocky 10 OS-es
...
curl -O https://rpm.frrouting.org/repo/$FRRVER-repo-1-0.el10.noarch.rpm
dnf install -y ./$FRRVER*

# install FRR
dnf install -y frr frr-pythontools epel-release kernel-modules-extra vim git-all iptables-services

dnf install -y libreswan openvpn easy-rsa strongswan strongswan-libipsec strongswan-charon-nm
